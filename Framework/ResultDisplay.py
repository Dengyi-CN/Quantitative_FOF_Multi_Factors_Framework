...
if __name__ == "__main__":
    ...
    # 测试
    output_file_url = 'D:/Quantitative_FOF_Framework_data/ResultDisplay_20190110'
    ...
    factor_return_reg_result_sql = 'select ' + ','.join(factor_return_reg_columns) + ' from lyzs_tinysoft.factor_return_regression'

    factor_return_reg_result = read_data_from_oracle_db(factor_return_reg_result_sql).rename(
        columns={col.upper():col for col in factor_return_reg_columns})[factor_return_reg_columns]
    ...
    for primary_key in primary_keys_list:
        ...
        for rolling_num in week_data_rolling_pct_list:
            temp_ts_pct_melt_data['last_' + str(rolling_num) + '_week'] = np.nan
            rolling_result = temp_ts_pct_melt_data.groupby(by=['type_name']).apply(
                lambda df: df['alpha'].rolling(rolling_num, min_periods=1).apply(
                    lambda alpha: len(pd.Series(alpha).dropna()) / len(alpha))).reset_index()
            temp_ts_pct_melt_data.loc[rolling_result['level_1'], 'last_' + str(rolling_num) + '_week'] = rolling_result['alpha'].values
