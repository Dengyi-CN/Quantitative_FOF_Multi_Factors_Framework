...
def get_factor_info():
    ...
    factor_first_class_dict = {factor_library.loc[i, 'FACTOR_NUMBER']: factor_library.loc[i, 'FIRST_CLASS'] for i in
                            range(factor_library.shape[0])}
...
if __name__ == "__main__":
    ...
    for primary_key in primary_keys_list:
        ...
        temp_ts_pct_pivot_data[temp_ts_pct_pivot_data.columns] = signifcant_factor_return_reg[signifcant_factor_return_reg[primary_keys_name_list].isin(
            primary_key).all(1)].pivot_table(index='get_data_date', columns='type_name', values='alpha')[temp_ts_pct_pivot_data.columns]
    ...
    for primary_key in primary_keys_list:
        ...
        temp_rank_pivot_data[temp_rank_pivot_data.columns] = \
            ranking_alpha_data[ranking_alpha_data[primary_keys_name_list].isin(primary_key).all(1)].pivot_table(
                index='get_data_date', columns='type_name', values='rank')[temp_rank_pivot_data.columns]
    ...
    ranking_sql = '''
        SELECT ''' + ','.join(ranking_columns) + ''',
            rank () OVER (PARTITION BY REGRESSION_MODEL, ROLLING_WINDOW, GET_DATA_DATE, FACTOR_NUMBER ORDER BY R_SQUARED DESC) rank
        FROM LYZS_TINYSOFT.FACTOR_RETURN_REGRESSION
        WHERE ALPHA_P_VALUE <= 0.1
    '''
    ...
    for primary_key in primary_keys_list:
        ...
        temp_rank_pivot_data = pd.DataFrame(index=all_get_data_date, columns=['第' + str(i + 1) + '档收益率' for i in range(10)])
        temp_rank_pivot_data[temp_rank_pivot_data.columns] = \
            ranking_rsquared_data[ranking_rsquared_data[primary_keys_name_list].isin(primary_key).all(1)].pivot_table(
                index='get_data_date', columns='type_name', values='rank')[temp_rank_pivot_data.columns]
        temp_rank_melt_data = temp_rank_pivot_data.reset_index().rename(columns={'GET_DATA_DATE': 'get_data_date'}).melt(
            id_vars=['get_data_date'], var_name=['type_name'], value_name='rank')
